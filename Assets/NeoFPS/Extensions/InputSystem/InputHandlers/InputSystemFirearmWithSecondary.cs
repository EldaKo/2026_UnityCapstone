...
namespace NeoFPS
{
    [HelpURL("https://docs.neofps.com/manual/inputref-mb-inputsystemfirearmwithsecondary.html")]
    public class InputSystemFirearmWithSecondary : FpsInputSystemHandler
    {
#if ENABLE_INPUT_SYSTEM
        ...
        void ApplyFirearmInput(ModularFirearm firearm, InputAction fireButton, bool reload, bool switchModes)
        {
        ...
        void ApplyWieldableToolInput(WieldableTool tool, InputAction fireButton, bool reload)
        {
            if (GetButtonDown(fireButton))
                tool.PrimaryPress();
            if (GetButtonUp(fireButton))
        ...
        void ApplyMeleeInput(BaseMeleeWeapon melee, InputAction fireButton)
        {
            if (GetButtonDown(fireButton))
                melee.PrimaryPress();
            if (GetButtonUp(fireButton))
        ...
        void ApplyThrownInput(BaseThrownWeapon thrownWeapon, InputAction fireButton)
        {
            if (GetButtonDown(fireButton))
        ...
        protected override void UpdateInput()
        {
            ...
            bool reload = GetButtonDown(controls.Combat.Reload);
            bool switchModes = GetButtonDown(controls.Combat.SwitchWeaponModes);

            ApplyFirearmInput(m_Firearm, controls.Combat.PrimaryFire, reload, switchModes);
            ApplyInput(m_Secondary, controls.Combat.SecondaryFire, reload, switchModes);
        ...
        void ApplyInput(WeaponInfo weaponInfo, InputAction fireButton, bool reload, bool switchModes)
        {
