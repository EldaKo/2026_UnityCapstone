...
namespace NeoFPS
{
    ...
    [HelpURL("https://docs.neofps.com/manual/inputref-mb-inputsystemfirearm.html")]
    public class InputSystemFirearm : FpsInputSystemHandler
    {
        ...
        protected override void OnEnable()
        {
			...
			m_EnabledFiring = GetButton(NeoFpsNewInputManager.controls.Combat.PrimaryFire);
        ...
        protected override void UpdateInput()
		{
			...
			// Fire
			if (GetButtonDown(controls.Combat.PrimaryFire) || (m_EnabledFiring && GetButton(controls.Combat.PrimaryFire)))
            {
			...
			if (GetButtonUp (controls.Combat.PrimaryFire))
				m_Firearm.trigger.Release();
			if (GetButtonDown (controls.Combat.SwitchWeaponModes))
			...
			// Optics
			if (GetButtonDown(controls.Combat.OpticsBrightnessUp))
			{
				var optics = GetComponentInChildren<IOpticsBrightnessControl>(false);
				if (optics != null)
					optics.IncrementBrightness();
			}
            if (GetButtonDown(controls.Combat.OpticsBrightnessDown))
            {
                var optics = GetComponentInChildren<IOpticsBrightnessControl>(false);
                if (optics != null)
			...
			if (m_Inspect != null)
			{
				if (m_Inspect.toggle)
				{
					if (GetButtonDown(controls.Inventory.InspectWeapon))
