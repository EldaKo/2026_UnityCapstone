...
namespace NeoFPS
{
    ...
    [HelpURL("https://docs.neofps.com/manual/inputref-mb-inputsystemcharactermotion.html")]
    public class InputSystemCharacterMotion : CharacterInputHandlerBase
    {
        ...
        protected override void UpdateInput()
        {
            ...
            if (allowLookInput)
            {
                m_Aimer.HandleMouseInput(GetVector2(controls.Movement.MouseLook) * 0.05f);
                m_Aimer.HandleAnalogInput(GetVector2(controls.Movement.AnalogueLook));
            }

            // Movement input
            if (allowMoveInput)
            {
                Vector2 move = GetVector2(controls.Movement.AnalogueMove);
                if (GetButton(controls.Movement.Forwards))
                    move.y += 1f;
                if (GetButton(controls.Movement.Backwards))
                    move.y -= 1f;
                if (GetButton(controls.Movement.StrafeLeft))
                    move.x -= 1f;
                if (GetButton(controls.Movement.StrafeRight))
                    move.x += 1f;
                ...
                // Movement modifiers
                bool sprintPress = GetButtonDown(controls.Movement.SprintToggle);
                bool sprintHold = GetButton(controls.Movement.Sprint);
                bool crouchPress = GetButtonDown(controls.Movement.CrouchToggle);
                bool crouchHold = GetButton(controls.Movement.Crouch);
                ...
                if (m_SprintHoldProperty != null)
                    m_SprintHoldProperty.on = sprintHold || GetButton(controls.Movement.SprintToggle);
                ...
                if (m_CrouchHoldProperty != null)
                    m_CrouchHoldProperty.Hold(crouchHold || GetButton(controls.Movement.CrouchToggle));
                ...
                if (m_JumpHoldProperty != null)
                    m_JumpHoldProperty.Hold(GetButton(controls.Movement.Jump));

                // Dodge
                if (m_EnableDodging && m_DodgeLeftTrigger != null && m_DodgeRightTrigger != null)
                {
                    if (GetButtonDown(controls.Movement.StrafeLeft))
                    {
                    ...
                    if (GetButtonDown(controls.Movement.StrafeRight))
                    {
                ...
                {
                    if (GetButtonDown(controls.Combat.Ability))
                ...
                if (m_BodyLean != null)
                {
                    if (m_ToggleLean)
                    {
                        if (GetButtonDown(controls.Movement.LeanLeft))
                        {
                        ...
                        if (GetButtonDown(controls.Movement.LeanRight))
                        {
                            if (Mathf.Abs(m_BodyLean.targetLean) > 0.9f)
                                m_BodyLean.ResetLean();
                            else
                    ...
                    {
                        int lean = 0;
                        if (GetButton(controls.Movement.LeanLeft))
                            --lean;
                        if (GetButton(controls.Movement.LeanRight))
                            ++lean;
            ...
            {
                if (GetButtonDown(controls.Interaction.Use))
                    m_InteractionMgr.InteractPress();
                if (GetButtonUp(controls.Interaction.Use))
