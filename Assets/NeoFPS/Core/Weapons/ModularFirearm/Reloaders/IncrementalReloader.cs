...
namespace NeoFPS.ModularFirearms
{
    ...
    public class IncrementalReloader : BaseReloaderBehaviour
    {
        ...
        [SerializeField, Tooltip("If true, then the reload count animation sent to the animator controller will be 1 lower to prevent playing an extra animation loop.")]
        private bool m_StartIncludesReload = false;
        ...
        [SerializeField, AnimatorParameterKey(AnimatorControllerParameterType.Float, true, true), Tooltip("A float parameter in the weapon animator controller that will be assigned the reload speed (use for animation clip playback speed setting on reload animations).")]
        private string m_ReloadSpeedParameter = string.Empty;
        ...
        private int m_ReloadSpeedFloatHash = -1;
		...
		public int reloadCount
		{
			...
			set
			{
                ...
                int animatorCount = m_ReloadCount;
                if (m_StartIncludesReload)
                    animatorCount -= 1;

				// Set animator property
				if (gameObject.activeInHierarchy && m_ReloadCountHash != -1)
					firearm.animationHandler.SetInteger (m_ReloadCountHash, animatorCount);
			}
        }
        ...
        protected override void Start ()
		{
            ...
            if (!string.IsNullOrWhiteSpace(m_ReloadAnimTrigger))
                m_ReloadAnimTriggerHash = Animator.StringToHash(m_ReloadAnimTrigger);
            else
                m_ReloadAnimTriggerHash = -1;

            if (!string.IsNullOrWhiteSpace(m_ReloadAnimCountProp))
                m_ReloadCountHash = Animator.StringToHash(m_ReloadAnimCountProp);
            else
                m_ReloadCountHash = -1;

            if (!string.IsNullOrWhiteSpace(m_ReloadSpeedParameter))
                m_ReloadSpeedFloatHash = Animator.StringToHash(m_ReloadSpeedParameter);
            else
                m_ReloadSpeedFloatHash = -1;
        ...
        protected override void OnEnable()
        {
            ...
            if (m_ReloadCountHash != -1)
                firearm.animationHandler.SetInteger(m_ReloadCountHash, m_StartIncludesReload ? m_ReloadCount - 1 : m_ReloadCount);
        }
        ...
        protected override void OnReloadSpeedMultiplierChanged()
        {
            if (m_ReloadSpeedFloatHash != -1)
                firearm.animationHandler.SetFloat(m_ReloadSpeedFloatHash, reloadSpeedMultiplier);
        }
