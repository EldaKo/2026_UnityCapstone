...
namespace NeoFPS.ModularFirearms
{
    public abstract class BaseAimerBehaviour : BaseFirearmModuleBehaviour, IAimer, IFirearmModuleValidity, INeoSerializableComponent
    {
        ...
        private float m_AimSpeedMultiplier = 1f;
        public float aimSpeedMultiplier
        {
            get { return m_AimSpeedMultiplier; }
            set
            {
                m_AimSpeedMultiplier = Mathf.Clamp(value, 0.01f, 10f);
                aimDurationMultiplier = 1f / m_AimSpeedMultiplier;
                OnAimSpeedMultiplierChanged();
            }
        }

        protected float aimDurationMultiplier
        {
            get;
            private set;
        } = 1f;

        protected virtual void OnAimSpeedMultiplierChanged()
        { }
        ...
        private static readonly NeoSerializationKey k_AimSpeedKey = new NeoSerializationKey("aimSpeed");
        ...
        public virtual void WriteProperties(INeoSerializer writer, NeoSerializedGameObject nsgo, SaveMode saveMode)
        {
            if (saveMode == SaveMode.Default)
            {
                ...
                writer.WriteValue(k_AimSpeedKey, m_AimSpeedMultiplier);
            }
        }
        ...
        public virtual void ReadProperties(INeoDeserializer reader, NeoSerializedGameObject nsgo)
        {
            ...
            if (reader.TryReadValue(k_AimSpeedKey, out float multiplier, m_AimSpeedMultiplier))
                aimSpeedMultiplier = multiplier;
        }
    }
}