...
namespace NeoFPS.ModularFirearms
{
	...
	public class AnimOnlyAimer : BaseAimerBehaviour
    {
        ...
        [SerializeField, AnimatorParameterKey(AnimatorControllerParameterType.Float, true, true), Tooltip("The animator parameter key for a float parameter used to modify the aim up/down speed.")]
        private string m_AimSpeedAnimFloat = "";
        ...
        {
            get { return m_AimTime * aimDurationMultiplier; }
        }
        ...
        protected override void Awake()
        {
            ...
            if (!string.IsNullOrEmpty(m_AimSpeedAnimFloat))
                m_AimSpeedHash = Animator.StringToHash(m_AimSpeedAnimFloat);
        }
        ...
        IEnumerator AimCoroutine(bool up)
        {
            ...
            if (up)
            {
                ...
                {
                    yield return null;
                    m_Lerp += Time.deltaTime * m_LerpMultiplier * aimSpeedMultiplier;
                    if (m_Lerp > 1f)
                    {
                        m_Lerp = 1f;
        ...
        protected override void OnAimSpeedMultiplierChanged()
        {
            // Set animator float parameter
            if (m_AimSpeedHash != -1 && firearm.animationHandler.isValid)
                firearm.animationHandler.SetFloat(m_AimSpeedHash, aimSpeedMultiplier);
        }
