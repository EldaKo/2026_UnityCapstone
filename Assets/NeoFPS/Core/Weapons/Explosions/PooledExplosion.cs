...
namespace NeoFPS
{
    ...
    public class PooledExplosion : MonoBehaviour, IDamageSource, INeoSerializableComponent
    {
        ...
        [SerializeField, Tooltip("If true then perform raycasts against overlapping colliders to check that there is no obstacle shielding them from the blast.")]
        private bool m_CheckVisibility = true;
        ...
        [SerializeField, Tooltip("If true then colliders will be grouped by health manager, and damage split across handlers that share one. This makes damage more consistent for characters with colliders on limbs, etc.")]
        private bool m_HandleCompoundColliders = true;
        ...
        public virtual void Explode(float maxDamage, float maxForce, IDamageSource source = null, Transform ignoreRoot = null)
        {
            ...
            // Split damage across handlers that share a root transform
            if (m_HandleCompoundColliders)
            {
                // Collate transforms
                ...
                for (int i = 0; i < s_DetectedDamageHandlers.Count; ++i)
                {
                    ...
                    var root = ddh.rootTransform;
                    if (root != null)
                    {
                        if (s_RootTransfomCounts.TryGetValue(root, out int count))
            ...
            for (int i = 0; i < s_DetectedDamageHandlers.Count; ++i)
            {
                if (s_DetectedDamageHandlers[i].damageHandler is Component)
            ...
            if (gatherDamangeHandlers)
            {
                ...
                for (int i = 0; i < s_DamageHandlers.Count; ++i)
                {
                    ...
                    for (int j = 0; j < s_DetectedDamageHandlers.Count; ++j)
                    {
                        ...
                        if (s_DetectedDamageHandlers[j].damageHandler == damageHandler)
                        {
                            if (s_DetectedDamageHandlers[j].falloff < falloff)
                                s_DetectedDamageHandlers[j] = new DamageHandlerInfo { damageHandler = damageHandler, falloff = falloff, damageShare = 1f, rootTransform = s_DetectedDamageHandlers[j].rootTransform };
                    ...
                    if (damageHandler != null)
                    {
                        ...
                        s_DetectedDamageHandlers.Add(new DamageHandlerInfo { damageHandler = damageHandler, falloff = falloff, damageShare = 1f, rootTransform = rootTransform });
                    }
                }
        ...
        protected virtual void ApplyExplosionForceEffect(ImpactHandlerInfo info, Vector3 center)
        {
            ...
            if (info.impactHandler is Component)
