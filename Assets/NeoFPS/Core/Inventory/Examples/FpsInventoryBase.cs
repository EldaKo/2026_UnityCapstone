...
namespace NeoFPS
{
	...
	public abstract class FpsInventoryBase : MonoBehaviour, IInventory, IQuickSlots, INeoSerializableComponent
	{
		...
		public abstract DuplicateEntryBehaviourFull duplicateBehaviour { get; }
        ...
        protected virtual void SwitchSelectionOnRemove(IInventoryItem removed)
        {
            if (!swapping)
            {
                if (m_SlotCount > 1)
                {
                    // Switch selection, or fall back to backup slot
                    if (!SwitchSelection() && backupSlot != null)
                        SetSelected(backupSlot, -1, false, false);

                    // Select nothing if switch selected removed
                    if (selected == removed)
                        SetSelected(null, -2, true, false);
                }
                else
                {
                    // Select starting slot and fall back to nothing
        ...
        public abstract SwapAction swapAction { get; }
        ...
        protected abstract bool IsEquippableItem(int slot);
            ...
            if (selected != null)
            {
                if (m_SlotCount > 1)
                {
                    ...
                    for (int i = 0; i < m_SlotCount; ++i)
                    {
                        ...
                        if (IsEquippableItem(index) && SelectSlot(index))
                            return true;
                    }
		...
		public void DropItem (IQuickSlotItem qsi)
        {
            var item = qsi as FpsInventoryItemBase;
            if (item != null && item.inventory as FpsInventoryBase == this && qsi.isDroppable && qsi != backupSlot)
			{
