    ...
    public class FpsInventoryWieldable : FpsInventoryItemBase, IQuickSlotItem
    {
        ...
        public override void OnAddToInventory(IInventory i, InventoryAddResult addResult)
        {
            ...
            if (addResult == InventoryAddResult.Full && m_QuickSlot != -1)
            {
                ...
                bool forceAutoSwitch = false;
                if (old != null)
                {
                    if (old.quickSlot == m_QuickSlot)
                    {
                        switch (i.duplicateBehaviour)
                        {
                            case DuplicateEntryBehaviourFull.DestroyOld:
                                i.RemoveItem(old as IInventoryItem);
                                Destroy(old.gameObject);
                                break;
                            case DuplicateEntryBehaviourFull.DropOld:
                                if (old.isDroppable)
                                {
                                    var qs = i as IQuickSlots;
                                    qs.DropItem(old);
                                }
                                break;
                            default:
                                if (fpsInventory.selected == old)
