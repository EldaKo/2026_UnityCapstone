...
namespace NeoFPS
{
    ...
    public class CameraSeeker : AiSeeker, IHealthManager
	{
        ...
        [SerializeField, FormerlySerializedAs("m_MaxAngles"), Tooltip("The maximum angles the camera can reach (negative y is down).")]
        private Vector2 m_DetectionAngles = new Vector2(90, 45);
		...
		protected override void OnValidate ()
		{
			...
			m_DetectionAngles.x = Mathf.Clamp(m_DetectionAngles.x, 5f, 179f);
            m_DetectionAngles.y = Mathf.Clamp(m_DetectionAngles.y, 5f, 179f);
            // Check range minimum
		...
		protected override void Start ()
		{
			...
			m_FustrumNormals [0] = Quaternion.Euler (0f, -m_DetectionAngles.x, 0f) * Vector3.right;
			m_FustrumNormals [1] = Quaternion.Euler (0f, m_DetectionAngles.x, 0f) * Vector3.left;
			m_FustrumNormals [2] = Quaternion.Euler (m_DetectionAngles.y, 0f, 0f) * Vector3.up;
			m_FustrumNormals [3] = Quaternion.Euler (-m_DetectionAngles.y, 0f, 0f) * Vector3.down;

			// Check if yaw should be limited
			m_LimitYaw = (m_MaxAngles.x - m_MinAngles.x <= 359f);
			...
			while (true)
			{
				...
				else
				{
					if (moving)
					{
                        ...
                        {
                            RotateTowards(m_IdleRotations[m_CurrentIdleTarget], m_RotationSpeedIdle);
		...
		protected override IEnumerator SuspiciousCoroutine ()
		{
			...
			while (true)
			{
				...
				else
				{
					...
					{
                        RotateTowards(GetRotationToTarget(), m_RotationSpeedHostile);
					}
				}
			}
		}
			...
			while (true)
			{
				...
				else
				{
					...
					else
					{
                        ...
                        {
                            RotateTowards(m_TargetRotation, m_RotationSpeedHostile);
		...
		protected override IEnumerator DeadCoroutine ()
		{
			...
			{
				yield return m_FixedUpdateYield;
                RotateTowards(deadRotation, m_RotationSpeedHostile);
			}
        ...
        {
			// Get the shortest delta angle
			var delta = new Vector2(
				Mathf.DeltaAngle(m_CurrentRotation.x, target.x),
                Mathf.DeltaAngle(m_CurrentRotation.y, target.y)
                );

			// Clamp based on speed
			float maxTurn = speed * Time.deltaTime;
			currentRotation += new Vector2(
				Mathf.Clamp(delta.x, -maxTurn, maxTurn),
                Mathf.Clamp(delta.y, -maxTurn, maxTurn)
                );
        }
