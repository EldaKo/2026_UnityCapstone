...
namespace NeoFPS
{
    ...
    public class TurretSeeker : CameraSeeker, IModularFirearm, IDamageSource
	{
		...
		private float m_ShootAngleThreshold = 5f;
		...
		protected override void OnValidate()
        {
			...
			m_ShootAngleThreshold = Mathf.Clamp(m_ShootAngleThreshold, 1f, 90f);
        }
   ...
   //         onStateChanged += (from, to) => 
        ...
        protected void FixedUpdate()
        {
            if (m_Firing)
			{
				if (state != State.Engaged || currentTarget == null || Vector3.Angle(lookTransform.forward, (currentTarget.position - lookTransform.position).normalized) > m_ShootAngleThreshold)
				{
					m_Firing = false;
					trigger.Release();
				}
