...
namespace NeoFPS
{
	...
	public class ScenePoolHandler : MonoBehaviour, INeoSerializableComponent
    {
        ...
        public void ReadProperties(INeoDeserializer reader, NeoSerializedGameObject nsgo)
        {
            ...
            var poolNSGOs = new List<NeoSerializedGameObject>();
            for (int i = 0; i < transform.childCount; ++i)
            {
                var child = transform.GetChild(i);
                var childNSGO = child.GetComponent<NeoSerializedGameObject>();
                if (childNSGO != null)
                    poolNSGOs.Add(childNSGO);
            }

            for (int i = 0; i < poolNSGOs.Count; ++i)
            {
                var poolEntry = poolNSGOs[i];
                var prefab = NeoSerializedObjectRegistry.GetPrefab(poolEntry.serializationKey);
                if (prefab != null)
                {
                    var pooledObject = prefab.GetComponent<PooledObject>();
                    if (pooledObject != null)
                    {
                        var activeNsgo = poolEntry.nestedObjects[1];
                        var inactiveNsgo = poolEntry.nestedObjects[-1];
