...
namespace NeoFPS
{
	...
	public class HudAmmoCounter : PlayerCharacterHudBase
	{
		...
		protected virtual void CheckReloaderType()
        {
            poolCurrentMax = m_Reloader is PassthroughReloader;
        }

        protected void OnReloaderChange (IModularFirearm firearm, IReloader reloader)
		{
			poolCurrentMax = false;
		...
		{
			if (m_MagazineCount != count && !poolCurrentMax)
			{
        ...
        protected void OnCurrentAmmoChange (IModularFirearm firearm, int count)
		{
			if (m_TotalCount != count)
			{
				if (poolCurrentMax)
                {
					...
					int max = firearm.ammo.maxAmmo;
                    m_TotalText.text = max.ToString();
                    m_TotalCount = max;
                }
				else
                {
