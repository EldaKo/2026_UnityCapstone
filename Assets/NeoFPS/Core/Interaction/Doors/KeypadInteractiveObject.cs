...
namespace NeoFPS
{
    ...
    public class KeypadInteractiveObject : InteractiveObject
    {
        ...
        [SerializeField, Tooltip("Should the passcode be randomised on start? The number of digits will not be changed from the passcode set above.")]
        private bool m_Randomise = false;
        ...
        [SerializeField, Tooltip("An event that is triggered when the correct code is entered.")]
        private UnityEvent m_OnCorrectCodeEntered = new UnityEvent();

        private bool m_PasscodeChanged = false;

        public void SetPassCodeDigit(int index, int value)
        {
            m_PassCode[index] = Mathf.Clamp(value, 0, 9);
            m_PasscodeChanged = true;
        }

        public void RandomisePassCode()
        {
            for (int i = 0; i < m_PassCode.Length; ++i)
                m_PassCode[i] = Random.Range(0, 10);
            m_PasscodeChanged = true;
        }
        ...
        private static readonly NeoSerializationKey k_PassCodeKey = new NeoSerializationKey("code");

        public override void WriteProperties(INeoSerializer writer, NeoSerializedGameObject nsgo, SaveMode saveMode)
        {
            base.WriteProperties(writer, nsgo, saveMode);

            if (m_PasscodeChanged)
                writer.WriteValues(k_PassCodeKey, m_PassCode);
        }
        ...
        public override void ReadProperties(INeoDeserializer reader, NeoSerializedGameObject nsgo)
        {
            ...
            if (reader.TryReadValues(k_PassCodeKey, out int[] values, null))
            {
                for (int i = 0; i < values.Length; ++i)
                    SetPassCodeDigit(i, values[i]);
            }
        }
    }
}