...
namespace NeoFPS
{
    public class FirstPersonCameraTransformConstraints : MonoBehaviour
    {
        ...
        private UpdateLoop m_UpdateLoop = UpdateLoop.FixedUpdate;
        ...
        private class StartingConstraints : IFirstPersonCameraPositionConstraint
        {
            ...
            {
                return Mathf.Lerp(0f, 0.1f / Time.fixedDeltaTime, m_Owner.m_PositionDamping);
            }

            public Vector3 GetConstraintPosition(Transform relativeTo)
            {
                m_DampedPosition = Vector3.Lerp(relativeTo.InverseTransformPoint(m_Owner.m_DefaultTarget.position), m_DampedPosition, dampingMultiplier * Time.deltaTime);
